...
- name: Blockstack
  ...
  full_description: Blockstack is an application stack for decentralized, server-less apps secured by the blockchain. Blockstack provides services for naming, identity, authentication, and storage without trusting any third-parties.
  links:
  - name: ''
...
- name: Decentralized Autonomous Society
  people:
  - name: Joel Dietz
  - name: Philip Saunders
  - name: Moritz Bierling
  - name: Tristan Roberts
  - name: Dakota Kaiser
  short_description: Enabling autonomy via decentralized technology
  full_description: The Decentralized Autonomous Society is a distributed thinktank that also has regular meetups in Palo Alto. It has the broad goal of facilitating new types of governance that are made possible via decentralized technologies. It was founded in the early days of Ethereum.
  links: []
- name: ETH CORE LIMITED
  people:
  - name: Dr. Gavin Wood
  - name: Dr. Jutta Steiner
  - name: Kenneth Kappler
  - name: TJ Saw
  - name: Dr. Aeron Buchanan
...
- name: Tahoe-LAFS
  ...
  full_description: 'Tahoe-LAFS (https://tahoe-lafs.org) is a system to store files and<br/>directories across multiple untrusted servers. Data is redundantly<br/>erasure-coded to be available even if some servers fail, and is encrypted to<br/>prevent server operators from learning or modifying your files.<br/><br/>Tahoe was first created for an online personal-backup company (AllMyData, now<br/>defunct). The 1.0 open-source release was in 2008, and the latest (1.11.0)<br/>was in March 2016. Development is fairly active.<br/><br/>Tahoe-LAFS has an apparently-unique capability-based access control mechanism<br/>with no central administrators, identities, or ACLs. Each uploaded file gets<br/>a cryptographic "filecap" string, which is both necessary and sufficient to<br/>download the file. Servers do not learn filecaps. The system provides<br/>(im)mutable files and directories. The directed graph of file objects enables<br/>easy sharing topologies (although the approach feels foreign to many people).<br/><br/>Tahoe is written in Python, and has a basic HTTP API, a web-based directory<br/>browser, CLI tools, and FTP/SFTP frontends. Several backup tools have added<br/>support for Tahoe-LAFS as a backend.<br/><br/>Tahoe users join a "grid", which is a particular set of clients and storage<br/>servers that connect to and hold data for each other. Each grid uses a<br/>central "Introducer" to distribute connectivity information.<br/><br/>We have several deployment styles in mind:<br/><br/>* "personal grid": e.g. my laptop, desktop, and a rented VPS server. This is<br/>  a personal backup solution: if my laptop breaks, I recover from the<br/>  desktop, and if the house burns down, I recover both (slowly) from the VPS.<br/><br/>* "friendnet": a group of people configure their boxes as client+server, and<br/>  store data on each other. This is a group backup solution, and is handy for<br/>  sharing files among group members too.<br/><br/>* "commercial net": an individual pays a company to run a storage server for<br/>  them. Paid backup solution.<br/><br/>We''d love to run the "one true grid" (unbounded size, where all users of<br/>Tahoe would join the same grid), and it would be easier to explain. But we<br/>don''t know how to make that work. Scaling connections and repair work is one<br/>problem, but tragedy-of-the-commons is the larger one. I currently expect<br/>most grids to include at most a few hundred machines.<br/><br/>Our philosophy is that you are probably more than capable of providing your<br/>own resources, but if you want to pay someone for service, that should be<br/>easy. And running a server should be almost as easy as running a client.'
  links:
  - name: ''
    link: https://tahoe-lafs.org/
  - name: ''
    link: https://github.com/tahoe-lafs/tahoe-lafs
- name: The Hyperledger Project
  people:
  - name: Brian Behlendorf
  short_description: Blockchain technology stack focused on pluggable consensus mechanisms, smart contract platforms, and security models
  full_description: The Hyperledger Project aims to be a home for a collection of blockchain technology projects, aimed at addressing a set of needs faced by financial institutions, emerging-market inclusive finance projects, healthcare, and other use cases where permissioned-chain architectures may be beneficial, hopefully in ways applicable to all blockchain efforts.  There is an initial code base and a fair bit of momentum from nearly 50 member companies and consortia who have joined the project.
  links:
  - name: ''
